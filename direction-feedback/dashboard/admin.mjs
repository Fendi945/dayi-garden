...
let session,refreshPromise,days=7,filter='pending',offset=0,loading=false,deliveryOrder=null,previewUrl=null;
...
async function orders(append=false){
 ...
 try{if(!append){offset=0;$('orders').replaceChildren();}const x=await api('admin_business_orders',{offset,filter});if(!append&&!x.orders.length)$('orders').innerHTML='<p class="empty">这里暂时没有订单。</p>';
 ...
 const label=complete?'已交付':o.process_status==='completed'?'历史反馈待补齐':statusNames[o.process_status]||'待跟进';
 el.innerHTML=`<div class="order-top"><b class="order-code">${esc(o.order_code)}</b><span class="amount">¥${Number(o.amount_cny||0).toFixed(2)}</span></div><div class="order-meta"><span class="badge">${esc(label)}</span><span>${paid?'已确认到账':'待核对到账'}</span><span>${esc(time(o.created_at))}</span></div><p><b>${esc(o.contact||'未留联系方式')}</b></p><p class="sub">${esc(o.style)} · ${esc(o.yard_size)} · ${esc((o.needs||[]).join('、'))}</p>${o.notes?'<p class="sub">'+esc(o.notes)+'</p>':''}<p class="sub">现场照片：${o.has_photo?'已提交':'待补齐'} · 付款凭证：${o.has_proof?'已提交':'待补齐'}${paid?' · 到账确认：'+esc(time(o.paid_at)):''}</p>${o.process_status==='failed'?'<p class="error">这笔反馈需要继续跟进，订单和款项记录已保留。</p>':''}<details data-materials><summary>查看现场与付款凭证</summary><div class="detail-grid" data-images></div></details>${o.image_url?'<img class="detail-img" src="'+esc(o.image_url)+'" alt="本次反馈方向图" loading="lazy">':''}${o.advice_json?.length?'<ol>'+o.advice_json.map(a=>'<li>'+esc(a)+'</li>').join('')+'</ol>':''}<details data-history><summary>查看反馈交付记录</summary><div data-deliveries></div></details><div class="order-actions"></div>`;
 ...
 el.querySelector('[data-history]').addEventListener('toggle',async e=>{if(!e.target.open)return;try{const h=await api('admin_history',{order_code:o.order_code});el.querySelector('[data-deliveries]').innerHTML=h.deliveries.length?h.deliveries.map(d=>'<p class="sub">第 '+d.version+' 版 · '+esc(time(d.created_at))+'</p><img class="detail-img" src="'+esc(d.image_url)+'" alt="已交付方向图"><ol>'+d.advice.map(a=>'<li>'+esc(a)+'</li>').join('')+'</ol>').join(''):'<p class="sub">暂无历史版本；现有结果显示在订单中。</p>';}catch(err){message(err.message);}});
 const actions=el.querySelector('.order-actions');
 function action(label,fn){const b=document.createElement('button');b.className='action-primary';b.textContent=label;b.addEventListener('click',async()=>{b.disabled=true;try{await fn();}catch(e){message(e.message);}finally{b.disabled=false;}});actions.append(b);}
 if(o.payment_status==='pending_verification'&&o.has_photo&&o.has_proof&&o.contact)action('已核对实际到账，确认收款',async()=>{await api('admin_confirm',{order_code:o.order_code});message('到账已确认并保存。');await Promise.all([metrics(),orders()]);});
 if(paid&&!o.production_active)action(complete?'更新反馈':'填写并交付反馈',()=>openDelivery(o));
 $('orders').append(el);}
 offset+=x.orders.length;$('more').classList.toggle('hidden',x.orders.length<25);
 }finally{loading=false;}
}
async function openDelivery(o){
 const imgs=await api('admin_order_images',{order_code:o.order_code});deliveryOrder=o;$('deliveryForm').reset();$('deliveryOrder').textContent=o.order_code+' · '+o.contact;$('deliveryOriginal').src=imgs.yard;$('deliveryPreview').hidden=true;$('deliveryMessage').textContent='';[1,2,3].forEach((n,i)=>$('advice'+n).value=o.advice_json?.[i]||'');$('deliveryDialog').showModal();
}
